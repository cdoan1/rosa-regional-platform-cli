...
func newDeleteCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Delete an S3-backed OIDC issuer",
		Long: `Delete an S3-backed OIDC issuer and its IAM OIDC provider.

This command invokes the OIDC deletion Lambda function to:
1. Delete all objects in the S3 bucket
2. Delete the S3 bucket
3. Delete the IAM OIDC provider

Bucket naming: The provided name will be automatically prefixed with "oidc-issuer-"
(e.g., "my-cluster" becomes "oidc-issuer-my-cluster").`,
		Example: `  # Delete OIDC issuer (auto-prefixed to oidc-issuer-my-cluster)
  rosactl oidc delete my-cluster

  # Delete OIDC issuer in specific region
  rosactl oidc delete my-cluster --region us-west-2

  # Use a different Lambda function name
  rosactl oidc delete my-cluster --function my-oidc-delete-lambda`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Invoke OIDC deletion Lambda function
			fmt.Printf("Invoking OIDC deletion Lambda function '%s'...\n", functionName)
	...
	cmd.Flags().StringVar(&functionName, "function", "oidc-delete", "Name of the OIDC deletion Lambda function to invoke")
